...
class ClarificationLogORM(Base):
    ...
    user_id: Mapped[SAUUID] = mapped_column(SAUUID(as_uuid=True), nullable=False, index=True)
    ...
    # Ambiguity Detection
    ambiguity_score: Mapped[float] = mapped_column(Numeric(3, 2), nullable=False)
    ambiguity_type: Mapped[str] = mapped_column(String(50), nullable=False)

    # Decision Tracking
    decision: Mapped[str] = mapped_column(String(10), nullable=False)  # "clarify" atau "direct"
    decision_source: Mapped[str] = mapped_column(String(20), nullable=False)  # "rules", "llm", atau "llm_fallback"

    # Clarification Data
    clarifying_question: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    clarification_answer: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    disambiguated_query: Mapped[Optional[str]] = mapped_column(Text, nullable=True)

    # User Feedback (untuk evaluasi)
    user_feedback: Mapped[Optional[bool]] = mapped_column(Boolean, nullable=True)
    needed_correction: Mapped[Optional[bool]] = mapped_column(Boolean, nullable=True)
